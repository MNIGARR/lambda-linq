...
List<Debtor> debtors = new List<Debtor> {
            ...
            new Debtor("James E. Denning", DateTime.Parse("May 4, 1988"), "504-289-8613", "JamesEDenning@jourrapide.com", "1444 Rose Avenue Metairie, LA 70001", 8176),
...
//var result = debtors.Where(db =>
//{
//    var email = db.Email;
//    return email.EndsWith("rhyta.com") || email.EndsWith("dayrep.com");
//}).ToList();
...
//var ageresult = debtors.Where(db =>
//{
//    var age = DateTime.Now.Year - db.BirthDay.Year;
//    return age > 26 && age < 36;
//}).ToList();
//ageresult.ForEach(d => Console.WriteLine(d.FullName + d.BirthDay));



//  3) Borcu 5000 - den cox olmayan borclularic cixartmag
//var debtresult = debtors.Where(db => db.Debt < 5000).ToList();
//debtresult.ForEach(d => Console.WriteLine(d.FullName + " " +d.Debt));



//	4) Butov adi 18 simvoldan cox olan ve telefon nomresinde 2 ve ya 2-den cox 7 reqemi olan borclulari cixartmaq
//var dbtrs = debtors.Where(db => db.FullName.Length == 18 && db.Phone.Count(nb => nb == '7') >= 2).ToList();
//dbtrs.ForEach(d => Console.WriteLine(d.FullName + " " +d.Phone));
...
//var result = debtors.Where(db =>
//{
//    var month = db.BirthDay.Month;
//    return month == 12 || month == 1 || month == 2;
//}).ToList();
//result.ForEach(d => Console.WriteLine(d.FullName + " " + d.BirthDay));
...
//var dbtrs = debtors.Where(db => db.Debt > debtors.Average(dbt => dbt.Debt))
//                   .OrderByDescending(nm=> nm.FullName)
//                   .OrderByDescending(dbts => dbts.Debt)
//                   .ToList();
//dbtrs.ForEach(d => Console.WriteLine(d.FullName + " " + d.Debt));
//soyadlara gore siralaya bilmedim:(



//	7) Telefon nomresinde 8 olmayan borclularin yalniz familyasin, yashin ve umumi borcun meblegin cixarmaq
//var result = debtors.Select(db => $"{db.FullName} {db.Phone} {db.Debt}")
//                    .Where(phone => phone.Contains("8")).ToList();
//result.ForEach(db => Console.WriteLine(db));
...
//        {
//            if (db.FullName[i] == db.FullName[j]) count++;
//        }
//        if (count == 3)
//        {
//            return true;
...
//    return false;
//})
//    .Select(db => $"{db.FullName} {db.Debt}")
//    .ToList();
//dbtrs.ForEach(db => Console.WriteLine(db));
...
//var dbtrs = debtors.OrderByDescending(db => db.Debt)
//                   .Select(db => $"{db.FullName} {db.Debt}")
//                   .ToList();
//for (int i = 0;i < 5;)
//{
//    i++;
//    Console.WriteLine(dbtrs[i]);
//}
...
//result.ForEach(db => Console.WriteLine(db.FullName + db.Debt));


//	12)2ci dunya muharibesin gormush borclularin siyahisi cixartmaq
//var result = debtors.Where(db => db.BirthDay.Year <= 1942).ToList();
//result.ForEach(d => Console.WriteLine(d.FullName + d.BirthDay));
...
//var dbtrs = debtors.Where(db =>
//{
//    for (int i = 0; i < db.Phone.Length; i++)
//    {
//        int count = 0;
//        for (int j = 0; j < db.Phone.Length; j++)
//        {
//            if (db.Phone[i] == db.Phone[j]) count++;
//        }
//        if (count == 0)
//        {
//            return true;
...
//    return false;
//})
//    .Select(db => $"{db.FullName} {db.Phone} {db.Debt}")
//    .ToList();
//dbtrs.ForEach(db => Console.WriteLine(db));